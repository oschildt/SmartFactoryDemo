...
function connect_mysql()
{
    ...
    try {
        $dbw->execute_query("SELECT LENGTH(TEXT_DATA) LTD, LENGTH(BLOB_DATA) LBD FROM LARGE_DATA WHERE ID = 1");
    } catch (\Exception $ex) {
        messenger()->setError($ex->getMessage());
        return false;
    }
...
if (connect_mysql() === null) {
    echo "<h4 style='color: maroon'>Please ensure that you have created the demo database with the script 'database/create_database_mysql.sql' and adjust the DB password and other connection data in the line 26 of this file!</h4>";
} else {
    report_messages();
}
?>
